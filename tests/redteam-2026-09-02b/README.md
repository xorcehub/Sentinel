...
|----|------|--------|---------------|
| F1 | EXEC-001 | `powershell -ep b` / `-ExecutionPolicy B` / `-w h` — PS resolves abbreviated param names AND abbreviated enum values, so Bypass policy with zero keyword matches (`'-ep bypass'` is longer than `'-ep b'`, contains() can't catch any prefix form). Conhost broker variant too (EXEC-002 needs `--headless`). | Add prefix tokens: `-ep b`, `-ep u`, `-ep:B`, `-ep:U`, `-ExecutionPolicy B`, `-ExecutionPolicy:U`, `-exec b`, `-w h`, `-WindowStyle h`. Token lists are whack-a-mole against PS prefix matching — the durable fix is an engine helper that parses PS args. |
