...
| F3 | PERSIST-001 | `schtasks /create /tr "%TEMP%\upd.exe"` (env-var: no literal path token) and `schtasks /create /xml <Documents task.xml>` (action lives in the XML, not the cmdline) both skip `selection_path`. | **PARTIALLY FIXED**: `%temp%`/`%appdata%` tokens added (env-var row flipped to control). `/xml` residual documented: needs an engine helper resolving the XML; backstop BASE-001 daily autoruns diff (<=24h). |
| F4 | PERSIST-003 | User-writable autostart values outside the Run/RunOnce/Winlogon/AppInit/Userinit family: `...\Windows NT\CurrentVersion\Windows\Load`, `HKCU\...\Active Setup\Installed Components\{guid}\StubPath`. | **FIXED**: both tokens added. Anchored on `\Windows\Load` (not the README-suggested whole `CurrentVersion\Windows` key) so per-user Device/printer churn in the same key family stays quiet — pinned by TestF4AnchorKeepsDeviceChurnQuiet. Rows flipped to controls. |
